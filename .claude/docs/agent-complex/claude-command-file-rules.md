...
> 1. **YAML Header (REQUIRED for New Commands):** Command files MUST start with a YAML frontmatter block (surrounded by `---`) containing:
>    - **description** (REQUIRED): Command purpose shown in help text
>    - **arguments** (REQUIRED): Structured list of command arguments with names, descriptions, and required flags
>    - **version** (REQUIRED): Semantic version number (e.g., "0.1.0", "1.0.0")
> 2. **Legacy Format (Deprecated):** Only for maintaining old commands. Uses ``# Args: <arguments>. v0.0.1. Description.`` format on the first line.
...
Claude commands support two header formats:

#### Option A: YAML Frontmatter (Recommended for New Commands)

Claude command files use a YAML header (frontmatter) to provide metadata, focusing on specifying a `description` and structured `arguments`. This makes your commands clear, discoverable, and able to accept multiple inputs from the user.

**YAML Header Format:**
```yaml
---
description: "Updates project metadata and adds reviewer notes for the release process"
arguments:
  - name: version
    description: "The new version number for the release"
    required: true
  - name: reviewer
    description: "Name or GitHub handle of the code reviewer"
    required: true
  - name: changelog
    description: "Path to the changelog markdown file"
    required: false
  - name: _preview
    description: "# Args: `<version>` `<reviewer>` `[changelog]`. v1.0.0. Updates project metadata and adds reviewer notes for the release process"
    required: false
version: "1.0.0"
---

Update the project to version `$version`, add reviewer `$reviewer`, and (if provided) attach changelog from `$changelog`.

Follow release best practices and document any additional reviewer feedback in the output.
```

**YAML Header Fields:**

**Required Fields:**
- **description** (REQUIRED): Summarizes the command's purpose and appears in the command menu and help text for easy discoverability
- **arguments** (REQUIRED): A list of argument objects, each containing:
  - **name**: The variable name for use in the prompt (referenced as `$name`)
  - **description**: What the argument represents, displayed as help text
  - **required**: Boolean indicating if the command needs this argument provided
- **version** (REQUIRED): Command version number following semantic versioning (e.g., "1.0.0", "0.1.0")
  - Start new commands at `0.1.0` for initial functional version
  - Use semantic versioning: `MAJOR.MINOR.PATCH`
  - Increment appropriately based on changes

**Optional Fields:**
- **category**: Logical grouping for command organization (e.g., "Git", "Development", "Testing")
- **allowed-tools**: List of tools the command can use (e.g., shell, python, webfetch)
- **icon**: Emoji or icon for the command (e.g., "🚀")

**How Arguments Are Used:**
- Reference each argument in the command body using `$argumentName`
- Claude CLI will prompt the user for required arguments
- Optional arguments can be provided but aren't mandatory
- For positional arguments (e.g., `$1`, `$2`), structure your list to match the order

#### Preview Display Workaround (IMPORTANT)

**Known Issue**: Claude Code currently displays the **last argument's description** as the preview text in the slash command dropdown, NOT the YAML `description` field. This is a known limitation (GitHub Issue #1370).

**Workaround Solution**: Add a dummy `_preview` argument as the LAST argument with a description that mimics the legacy format:

```yaml
arguments:
  - name: actual_arg1
    description: "Your real first argument"
    required: true
  - name: actual_arg2
    description: "Your real second argument"
    required: false
  - name: _preview
    description: "# Args: `<arg1>` `[arg2]`. v1.0.0. Your command's main description here"
    required: false
```

**Key Points:**
- The `_preview` argument is purely for display purposes
- Don't reference `$_preview` in your command implementation
- The description should include the full legacy format: `# Args:`, arguments, version, and description
- Mark it as `required: false` so it doesn't affect usage
- The underscore prefix indicates it's internal/dummy

This ensures your command displays properly in the slash command preview while maintaining structured YAML metadata.

**Example YAML Headers:**

```yaml
---
description: "Create a new user account with sudo and developer access"
arguments:
  - name: username
  ...
  - name: email
    ...
    required: true
  - name: groups
    description: "Additional groups to add the user to (comma-separated)"
    required: false
  - name: _preview
    description: "# Args: `<username>` `<email>` `[groups]`. v0.1.0. Create a new user account with sudo and developer access"
    required: false
version: "0.1.0"
...
```yaml
---
description: "Deploy edge functions to Supabase with optional environment variables"
arguments:
  - name: function_name
  ...
  - name: env_file
    ...
    required: false
  - name: _preview
    description: "# Args: `<function-name>` `<environment>` `[env-file]`. v1.2.0. Deploy edge functions to Supabase with optional environment variables"
    required: false
version: "1.2.0"
allowed-tools:
  - shell
...
#### Option B: Legacy Markdown Format (Still Supported)

**⚠️ CRITICAL FORMATTING REQUIREMENT**: If not using YAML header, the first line of your command file MUST follow this exact format because **this is what Claude displays to users when they list available commands**:
...
#### Choosing Between YAML and Legacy Format

**Use YAML Format (STRONGLY RECOMMENDED):**
- Creating any new commands
- You need structured, self-documenting arguments
- You want clear argument validation (required vs optional)
- You need to restrict allowed tools
- You want to organize commands by category
- You want to add visual icons for better UX
- You need professional, maintainable commands

**Use Legacy Format (LIMITED USE):**
- Maintaining legacy commands that haven't been migrated
- Quick temporary commands (though YAML is still preferred)
- Backwards compatibility with old workflows
- Simple commands with no arguments

**Examples of CORRECT Legacy Headers:**
...
- **Deployment Structure**: Scripts are organized to `.claude/scripts/{topic}/` maintaining topic separation
- **Script References**: Commands use `.claude/scripts/{topic}/script-name.sh` (relative to project root)
- **Self-Contained Topics**: Each topic contains all its dependencies

#### Bundle Structure

1. **Key Characteristics**
   - **Topic-organized deployment**: Scripts are organized to `.claude/scripts/{topic}/`
   - **Topic-aware paths**: Commands reference scripts with `.claude/scripts/{topic}/` (relative to project root)
   - **Bundle deployment**: Organized by topic for clear separation
...
3. **Example Bundle: vercel-playwright**
   ```
   # Deployed structure:
   .claude/
   ├── commands/
...
4. **Deployment Process**
   - Topic commands deploy to `.claude/commands/<topic-name>/`
   - Bundle scripts are organized to `.claude/scripts/{topic}/`
   - Commands can be invoked after deployment (e.g., `/vercel-playwright:get-preview-url-with-access-key`)
   - **IMPORTANT**: Scripts maintain topic separation in `.claude/scripts/{topic}/` directories
...
### Example with YAML Header (Recommended)

```yaml
---
description: "Create a feature branch with proper setup and tracking"
arguments:
  - name: base_branch
    description: "The branch to base the feature from (must exist in repository)"
    required: true
  - name: feature_name
    description: "Name of the feature to implement"
    required: true
  - name: create_issue
    description: "Whether to create a GitHub issue for this feature (yes/no)"
    required: false
category: "Git"
allowed-tools:
  - shell
...
Create a feature branch based on `$base_branch` named `$feature_name`. $create_issue

## Usage

```bash
/create-feature develop user-authentication
/create-feature main hotfix-login yes
```

## Implementation Notes

The branch will be created from `$base_branch` with the name `feature/$feature_name`.
If `$create_issue` is "yes", a corresponding GitHub issue will be created.
...
### Example with Legacy Header (Still Supported)
